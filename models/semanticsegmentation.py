...
class SemanticSegmentation(ModelBase):
    ...
    def load_model(self):
        model = torch.hub.load(self.hub_repo, self.modelName, pretrained=True)
        model.eval()
    ...
    @handle_alpha
    @torch.no_grad()
    def predict(self, input_image,modelName):
        self.modelName=modelName[1]
        self.name=modelName[0]
