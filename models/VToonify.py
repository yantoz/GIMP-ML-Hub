...
class VToonify(ModelBase):
    def __init__(self):
        super().__init__()
        self.hub_repo = "yantoz/VToonify"

    def load_model(self):
        if self.device != "cuda":
            os.environ['CUDA_VISIBLE_DEVICES'] = ""
        model = torch.hub.load(self.hub_repo, 'vtoonify',
            progress=True, map_location=self.device,
            style=self.style, style_id=self.style_id, style_degree=self.style_degree,
            scale_image=True, padding=[200,200,200,200], color_transfer=self.color_transfer)
        return model
